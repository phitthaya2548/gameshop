...
router.get("/accepted", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await conn.promise().query(sql, [riderId]);
    ...
    const transformedShipments = shipments.map(
      (s: {
        shipment_id: any;
        item_name: any;
        ...
        sender_avatar: any;
        send_address_id: any;
        send_address_text: any;
        receiver_name: any;
        receiver_phone: any;
        receiver_avatar: any;
        recv_address_id: any;
        recv_address_text: any;
        item_photo_url: any;
      }) => ({
...
function toNumStatus(v: unknown): number | null {
  const n = Number(String(v));
  return [1, 2, 3, 4].includes(n) ? n : null;
}
function nextOf(curr: number): number | null {
  if (curr === 1) return 2;
  if (curr === 2) return 3;
  if (curr === 3) return 4;
  return null;
}
function normalizeBase64(s: string): { b64: string; ext: "jpg" | "png" } {
  const t = s.trim();
...
router.post("/:shipmentId/advance", async (req, res) => {
  const shipmentId = Number(req.params.shipmentId);
  const riderId = Number(req.body?.rider_id);
  const photoBase64: string | undefined = req.body?.photo_base64; // รูปเดียว
  ...
  try {
    ...
    const assignedRider = rows[0].rider_id as number | null;
    ...
    const [updResult]: any = await conn.promise().query(
      `UPDATE Shipment
          SET status = ?,
              updated_at = NOW(),
              rider_id = CASE
    ...
    if (photoBase64 && photoBase64.trim()) {
      try {
        ...
        const publicPath = saveImageFromBase64(b64, ext); // e.g. "/uploads/2025/10/abc.jpg"

        await conn.promise().query(
          `INSERT INTO Shipment_Photo (shipment_id, rider_id, status, photo_url, uploaded_at)
           VALUES (?, ?, ?, ?, NOW())`,
          [shipmentId, riderId, String(next), publicPath]
        );
    ...
    // 4) ยืนยันสถานะล่าสุด
    const [afterRows]: any = await conn
      .promise()

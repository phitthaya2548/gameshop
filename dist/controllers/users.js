...
const user_addresses_1 = require("../models/user_addresses");
exports.router = express_1.default.Router();
exports.router.get("/:id/profile", async (req, res) => {
    try {
    ...
    `;
        const [addrRows] = await db_1.conn.promise().query(addrSql, [userId]);
...
exports.router.post("/:id/addresses", async (req, res) => {
    const userId = Number(req.params.id);
    if (!Number.isInteger(userId) || userId <= 0) {
        return res.status(400).json({ success: false, message: "id:int required" });
    }
    // ✅ ใช้ model
    const userAddress = new user_addresses_1.UserAddress(req.body, userId);
    ...
    try {
        ...
        if (userAddress.is_default) {
            await cx.query(`UPDATE User_Address SET is_default = FALSE WHERE user_id = ?`, [userAddress.user_id]);
        }
        const sql = `
...
exports.router.put("/:id/addresses/:addressId", async (req, res) => {
    const userId = Number(req.params.id);
...
exports.router.delete("/:id/addresses/:addressId", async (req, res) => {
    const userId = Number(req.params.id);
    const addressId = Number(req.params.addressId);
    try {
        const sql = `DELETE FROM User_Address WHERE address_id = ? AND user_id = ?`;
        await db_1.conn.promise().query(sql, [addressId, userId]);
...
exports.router.get("/:id/addresses", async (req, res) => {
    const userId = Number(req.params.id);
    if (!Number.isInteger(userId) || userId <= 0) {
        return res.status(400).json({ success: false, message: "id:int required" });
    }
    try {
      ...
      ORDER BY is_default DESC, address_id DESC
    `;
        const [rows] = await db_1.conn.promise().query(sql, [userId]);

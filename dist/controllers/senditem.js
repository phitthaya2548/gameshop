...
exports.router.get("/search", async (req, res) => {
    try {
      ...
      LIMIT 1
    `;
        const [senderRows] = await db_1.conn.promise().query(sql, [senderPhone]);
        const [receiverRows] = await db_1.conn.promise().query(sql, [receiverPhone]);
...
exports.router.post("/create", async (req, res) => {
    try {
        const { sender_id, receiver_id, pickup_address_id, delivery_address_id, item_name, item_description, photo_base64, // optional
        photo_url, // optional
         } = req.body || {};
        ...
        const cx = await db_1.conn.promise().getConnection();
        try {
            await cx.beginTransaction();
            const [result] = await cx.query(`INSERT INTO Shipment (
          sender_id, receiver_id, pickup_address_id, delivery_address_id,
          item_name, item_description, status
        ) VALUES (?, ?, ?, ?, ?, ?, '1')`, [sender_id, receiver_id, pickup_address_id, delivery_address_id, item_name, item_description]);
            ...
            if (typeof photo_url === "string" && photo_url.trim() !== "") {
                ...
                finalUrl = (0, upload_1.saveImageFromBase64)(photo_base64);
            }
            if (finalUrl) {
                await cx.query(`INSERT INTO Shipment_Photo (shipment_id, rider_id, status, photo_url, uploaded_at)
           VALUES (?, NULL, '1', ?, NOW())`, [shipmentId, finalUrl]);
            }
            await cx.commit();
...
exports.router.post("/create-batch", async (req, res) => {
    ...
    `;
        for (const rawItem of items) {
            const { receiver_id, delivery_address_id, item_name, item_description, photo_url, photo_base64, } = rawItem || {};
            if (!receiver_id || !delivery_address_id || !item_name || !item_description) {
                throw new Error("missing_item_fields");
            }
            const [result] = await cx.query(sqlInsertShipment, [
